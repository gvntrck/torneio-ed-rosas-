...
class ER_Shortcode
{
    ...
    public function render_shortcode($atts)
    {
        ...
        ob_start();
        ?>
        <div id="er-form-wrapper-<?php echo esc_attr($post_id); ?>" class="er-frontend-form-container"
            data-form-id="<?php echo esc_attr($post_id); ?>">
            <input type="hidden" name="er_tournament_form_id" value="<?php echo esc_attr($post_id); ?>">
            <input type="hidden" name="er_redirect_checkout" value="1">
                            ...
                            <?php foreach ($options as $index => $opt): ?>
                                <label class="er-radio-label">
                                    <input type="radio" name="<?php echo esc_attr($field_name); ?>"
                                        value="<?php echo esc_attr($opt['value']); ?>" data-price="<?php echo esc_attr($opt['price']); ?>"
                                        <?php echo $required_attr; ?>>
                                    <?php echo esc_html($opt['label']); ?>
                                    <?php echo $opt['price'] > 0 ? ' (+ R$ ' . number_format($opt['price'], 2, ',', '.') . ')' : ''; ?>
                                </label>
                            <?php endforeach; ?>
                        ...
                        <?php elseif ($field['type'] === 'checkbox' || $field['type'] === 'consent'): ?>
                            ...
                            <label class="er-checkbox-label">
                                <input type="checkbox" name="<?php echo esc_attr($field_name); ?>"
                                    value="<?php echo esc_attr($opt_val); ?>" data-price="<?php echo esc_attr($opt_price); ?>" <?php echo $required_attr; ?>>
                                <?php echo esc_html($opt_label); ?>
                                <?php echo $opt_price > 0 ? ' (+ R$ ' . number_format($opt_price, 2, ',', '.') . ')' : ''; ?>
                            </label>
